...
Inductive bq_ms se: bq_internal_state * rb_state -> Clight.state -> Prop :=
| bq_ms_enq:
  forall vf m m' v srb,
    Cop.val_casted v tint ->
    Genv.find_funct (Genv.globalenv se bq_program) vf = Some (Internal bq_enq) ->
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_init enq [v] m', srb) (Callstate vf [v] Kstop m)
| bq_ms_deq:
  forall vf m m' srb,
    Genv.find_funct (Genv.globalenv se bq_program) vf = Some (Internal bq_deq) ->
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_init deq [] m', srb) (Callstate vf [] Kstop m)

| bq_ms_call_inc1:
  forall vf k m' m b srb,
    vf = Vptr b Integers.Ptrofs.zero ->
    ...
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_call RBspec.inc1 [] m', srb) (Callstate vf [] k m)
| bq_ms_call_inc2:
  forall vf k m m' v b srb,
    Cop.val_casted v tint ->
    vf = Vptr b Integers.Ptrofs.zero ->
    ...
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_call RBspec.inc2 [v] m', srb) (Callstate vf [] k m)

| bq_ms_mid_enq:
  forall m m' v1 v2 k srb,
    ...
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_middle enq [v1; v2] m', srb) (Returnstate v1 k m)
| bq_ms_mid_deq:
  forall m m' v k srb,
    Cop.val_casted v tint ->
    k = (Kcall (Some bq_deq_tmp1) bq_deq empty_env
    ...
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_middle deq [v] m', srb) (Returnstate v k m)
...
| bq_ms_call_get:
  forall vf k m m' v b srb,
    ...
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_call RBspec.geti [v] m', srb) (Callstate vf [v] k m)
| bq_ms_call_set:
  forall vf k m m' v1 v2 b srb,
    vf = Vptr b Integers.Ptrofs.zero ->
    ...
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_call RBspec.seti [v1; v2] m', srb) (Callstate vf [v1; v2] k m)

| bq_ms_mid'_enq:
  forall m m' v k le srb,
    k = (Kcall None bq_enq empty_env le (Kseq (Sreturn None) Kstop)) ->
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_middle' enq v m', srb) (Returnstate v k m)
| bq_ms_mid'_deq:
  forall m m' v rv k srb,
    Cop.val_casted rv tint ->
    Cop.val_casted v tint ->
    k = (Kcall (Some bq_deq_tmp2) bq_deq empty_env
          (PTree.Node (PTree.Node PTree.Leaf (Some Vundef) PTree.Leaf) (Some v) PTree.Leaf)
    ...
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_middle' deq rv m', srb) (Returnstate rv k m)

| bq_ms_final:
  forall rv m m' srb,
    rb_match_mem se srb m ->
    Mem.nextblock m' = Mem.nextblock m ->
    bq_ms se (bq_final rv m', srb) (Returnstate rv Kstop m).
...
Opaque Nat.modulo.
Require Import Lia.
Opaque Genv.find_symbol.

Lemma bq_correct': forward_simulation rb_cc rb_cc (bq_spec @ rb_state) (Clight.semantics2 bq_program).
Proof.
  constructor. econstructor. reflexivity. firstorder.
  intros. instantiate (1 := fun _ _ _ => _). cbn beta.
  destruct H. subst.
  set (ms := fun (s1: state bq_spec * rb_state) s2 => bq_ms se2 s1 s2).
  eapply forward_simulation_plus with (match_states := ms).
  - intros.  destruct q1, s1. inv H; inv H1; inv H; cbn in *; subst.
    + eexists. split.
      * econstructor; eauto.
        eapply genv_funct_symbol; eauto. reflexivity.
        unfold type_of_function. f_equal; cbn.
        constructor; eauto. constructor. rewrite H7. eauto.
      * constructor; eauto. eapply genv_funct_symbol; eauto.
    + eexists. split.
      * econstructor; eauto.
        eapply genv_funct_symbol; eauto. reflexivity.
        unfold type_of_function. f_equal; cbn.
        constructor; eauto. rewrite H7. eauto.
      * constructor. eapply genv_funct_symbol; eauto. eauto. congruence.
  - intros. destruct r1, s1. inv H1; inv H2. cbn in *. subst. inv H.
    eexists. split. econstructor. constructor; eauto.
  - intros. destruct s1, q1.  inv H1. inv H2; cbn in *; subst; inv H.
    (* inc1 *)
    + eexists se2, _.  split. 2: split.
      * econstructor; eauto.
        eapply genv_funct_symbol; eauto. reflexivity.
      * assert (b = b0). cbn in *. congruence.
        subst. constructor. eauto. congruence.
      * split. eauto. intros.  destruct s1', r1, H1.
        cbn in *. subst. inv H. inv H1.
    ...
    + eexists se2, _.  split. 2: split.
      ...
      * split. eauto. intros. destruct s1', r1, H1.
        ...
        econstructor. apply H13. apply H7. reflexivity. eauto. congruence.
    (* set *)
    + eexists se2, _.  split. 2: split.
      * econstructor; eauto.
        eapply genv_funct_symbol; eauto. reflexivity.
      ...
      * split. eauto. intros. destruct s1', r1, H1.
        cbn in *. subst. inv H. inv H1.
        eexists. split. econstructor; eauto.
        econstructor. reflexivity. eauto. congruence.
  - intros. destruct s1'.  inv H1; inv H; inv H1.
