...
Module DCPOLCoalgTheory
  ...
  Module BW_FW : CategoryDefinition.
    Record bw_fw_sim (α β : coalg) :=
      mk_fw_sim {
        wit : coalg;
        bw : BW.bw_sim α wit;
        fw : FW.fw_sim wit β;
    ...
    Definition mor_to_bw_fw_sim {α β : coalg} (f : coalg_mor α β) : bw_fw_sim α β :=
      {|
        wit := α;
        bw := BW.id α;
        fw := FW.mor_to_fw_sim f;
      |}.

    Coercion mor_to_bw_fw_sim : coalg_mor >-> bw_fw_sim.

    Definition t : Type := C.t.

    Definition m (α β : t) : Type := bw_fw_sim α β.
